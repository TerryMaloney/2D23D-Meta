...
### Validating your real statements (do this before launch, and any time)
Put statement PDFs in `.private-statement-validation/input/` (the folder is
gitignored — nothing in it can ever be committed) and run:

```bash
npm run validate:statements
```

Every file is parsed by the production engine **locally**; the summary
(`results.html` in the same folder) shows per-file status, transaction
counts, and reconciliation differences — never any statement content. For a
file that fails, the converter UI's failure reporter produces an anonymized
layout, and `packages/parser/fixtures/layout-twin.ts` can turn that into a
committable synthetic regression fixture with internally consistent numbers.

### The Statement History Audit (`/audit`)
The multi-statement feature: users drop a folder of statements; the app
groups them by account, checks coverage gaps, duplicate statements/
transactions, and closing→opening balance continuity, finds recurring
payments and fees, and exports a merged CSV, a six-sheet XLSX audit
workbook, per-account QBO, and a printable report. All local, same privacy
guarantees. The "Try a sample 12-month audit" button uses the synthetic set
in `public/samples/audit/` (regenerated by `npm run fixtures`). Audit
exports respect the same free cap and license as single-statement exports.

### Continuous integration
`.github/workflows/verify.yml` runs lint, all unit/golden tests, both
builds, and the full Playwright suite (including the no-upload privacy
tests) on every push. The README badge shows current status. If the repo is
private, Actions minutes come from GitHub's free tier (~10–15 min per run).
